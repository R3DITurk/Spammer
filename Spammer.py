...
def banner():
    ...
    print()
    print(r"     Spammer: github.com/R3DITurk      ")
    print(RESET_ALL)
...
def main():
    banner()
    print("[1] SMS SPAMER.")
    print("[2] SPAMER'I GÜNCELLE.")
    print("[3] CIK.")
    print()
    number = input(f"{BRIGHT}{BLUE}Islem numarasını girin: {RESET_ALL}")
    if number == "1":
        spam_handler()
    elif number == "2":
        update()
        ...
        print(f"\n{BRIGHT}{RED}[*] Islem numarası yanlış girildi!{RESET_ALL}")
        sleep(1)
        main()
...
def spam_handler():
    ...
    check_version()
    banner()
    print("Telefon numarası girin")
    phone = parse_phone(input(f"{BRIGHT}{BLUE}Spammer >> {RESET_ALL}"))
    print()
    print("Proxy kullansınmı? (y/n)")
    ...
    try:
        ...
        threads = "1"
    banner()
    print(f"Telefon: {BRIGHT}{BLUE}{phone}{RESET_ALL}")
    print("Spam başlatıldı", end="\n")
    print(f"{BRIGHT}{RED}[*] Ctrl+Z İle Spamı Durdur{RESET_ALL}")
...
def parse_phone(phone):
    ...
    if len(phone) in [10, 11, 12]:
        ...
        print(f"\n{BRIGHT}{RED}[*] Telefon numarası yanlış girildi!{RESET_ALL}")
        sleep(1)
        spam_handler()
...
def check_internet():
    try:
        ...
        print(f"\n{BRIGHT}{RED}[*] İnternet bağlantısı yok!{RESET_ALL}")
        sleep(1)
        main()
    return
...
def check_version():
    version = "3.1"
    if float(version) < float(get("https://raw.githubusercontent.com/R3DITurk/Spammer/master/version.txt").text):
        print(f"\n{BRIGHT}{RED}[*] Sürüm güncel değil ve güncellenmesi gerekiyor!{RESET_ALL}")
        sleep(1)
        main()
    return
...
def update():
    check_internet()
    banner()
    print("Güncellemek istediğinize emin misiniz? (y/n)")
    ...
    if update.lower() == "y":
        ...
        try:
            file.write(get("https://raw.githubusercontent.com/R3DITurk/Spammer/master/spammer.py").content)
            file.close()
            system("spammer")
        except UnboundLocalError:
            system("cd $HOME && rm -rf spammer && git clone https://github.com/R3DITurk/Spammer && cd spammer && sh install.sh")
    else:
        main()
